...
static st_block_t st_block_buffer[SEGMENT_BUFFER_SIZE - 1] = {0};
...
static segment_t segment_buffer[SEGMENT_BUFFER_SIZE] = {0};
...
void st_wake_up()
{
  ...
  if (sys.state & (STATE_CYCLE | STATE_HOMING)) {
    ...
    // Timer3.attachInterrupt(timer3_handler).setPeriod(settings.pulse_microseconds).start();
...
//
  ...
  if (st.exec_segment == NULL) {
    ...
    if (segment_buffer_head != segment_buffer_tail) {
      ...
      Timer4.setPeriod(st.exec_segment->cycles_per_tick);  // cycles are now in uS... LDO
  ...
  while (segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
    ...
    if (pl_block == NULL) {
      ...
      if (prep.flag_partial_block) {
        ...
        if ( ++prep.st_block_index == (SEGMENT_BUFFER_SIZE - 1) ) {
...
//    }
      prep_segment->cycles_per_tick = cycles;  // BIG Number // LDO
