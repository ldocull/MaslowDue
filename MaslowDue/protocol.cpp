...
int protocol_main_loop()
{
  ...
  // ---------------------------------------------------------------------------------  
    ...
    while((pml_c = serial_read()) != SERIAL_NO_DATA) 
    {
      if ((pml_c == '\n') || (pml_c == '\r')) { // End of line reached
        ...
        if (comment != COMMENT_NONE) {
          ...
          // } else if (pml_c == '%') {
            ...
            comment = COMMENT_NONE;
            char_counter = 0;
          } else if (pml_c >= 'a' && pml_c <= 'z') { // Upcase lowercase
            line[char_counter++] = pml_c-'a'+'A';
          } else {
            line[char_counter++] = pml_c;
          }
        }
      }
    }
    ...
    if (sys.abort) { return -1; } // Bail to main() program loop to reset system.
    return 0;
}
  ...
  if (sys.state == STATE_SAFETY_DOOR) { 
    if (bit_istrue(sys.suspend,SUSPEND_ENABLE_READY)) { 
      if (!(system_check_safety_door_ajar())) {
       ...
       // Feed hold or something else has a com lock, so look for abort signal directly..
}  
