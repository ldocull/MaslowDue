...
//  MASLOW is circular in motion, so long lines must be divided up
    ...
    if((abs(deltax) > (float)(0.0)) || (abs(deltay) > (float)0.0))  // don't segment z-only moves
    {
      ...
      {  
        ...
        #ifdef USE_LINE_NUMBERS
          plan_buffer_line(cpos, feed_rate, invert_feed_rate, line_number);
        #else
          plan_buffer_line(cpos, feed_rate, invert_feed_rate);
        #endif
      }
    } 
    else
    {
        // DEBUG_COM_PORT.print("z-only move\n");
        #ifdef USE_LINE_NUMBERS
